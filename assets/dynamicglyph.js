Game.DynamicGlyph = function(properties) {
  ...
  // Create an object which will keep track of what mixins we have attached to
  // this entity based on the name property.
  ...
  for (var i = 0; i < mixins.length; i++) {
    // Copy over all properties from each mixin as long as its not the name,
    // init property or listners property. We also make sure not to override
    ...
    if (mixins[i].listeners) {
      for (var key in mixins[i].listeners) {
        // If we dont already have a key for this event in our listeners array, add it.
...
Game.DynamicGlyph.prototype.details = function() {
  ...
  // Iterate through each return value, grabbing the details form the arrays
  if (detailGroups) {
