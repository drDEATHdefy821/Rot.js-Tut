Game.Map = function(tiles, player) {
  ...
  this._fov = [];
  this.setupFov();
  // Create a list which will hold the entitys
  this._entities = [];
...
Game.Map.prototype.getTile = function (x, y, z) {
  ...
  if (x < 0 || x >= this._width || y < 0 || y >= this._height || z < 0 ||
      z >= this._depth) {
...
Game.Map.prototype.setupFov = function() {
  // Keep this in 'map' variable so that we dont lose it.
  var map = this;
  ...
  for (var z = 0; z < this._depth; z++) {
    ...
    (function () {
      // For each depth, we need to creat a callback which figures out if light
...
Game.Map.prototype.getEntityAt = function(x, y, z) {
  // Iterate through all entities searching for one with a matching position
...
Game.Map.prototype.addEntity = function(entity) {
  ...
  if (entity.getX() < 0 || entity.getX() >= this._width ||
      ...
      entity.setMap(this);
      // Add the entity to the list of entitys
...
Game.Map.prototype.removeEntity = function(entity) {
  // Find the entity in the list of entites if it is present
  for (var i = 0; i < this._entities.length; i++) {
