...
Game.EntityMixins.Destructible = {
  ...
  getDefenseValue: function() {
    var modifier = 0;
    // If we can equip items, then we have to take into
    // consideration weapon and armor
    if (this.hasMixin(Game.EntityMixins.Equipper)) {
      if (this.getWeapon()) {
...
Game.EntityMixins.Sight = {
  ...
  canSee: function(entity) {
    ...
    // If were not in a square field of view, then we wont be in a real
    // field of view either.
    if ((otherX - this._x) * (otherX - this._x) + (otherY - this._y) *
      (otherY - this._y) > this._sightRadius * this._sightRadius) {
        return false;
    }
...
Game.EntityMixins.FoodConsumer = {
  name: 'FoodConsumer',
  init: function(template) {
    this._maxFullness = template['_maxFullness'] || 1000;
  ...
  getHungerState: function() {
    ...
    } else if (this._fullscreen >= perPercent * 95) {
      return 'Oversatiated';
...
Game.EntityMixins.TaskActor = {
  ...
  hunt: function() {
    ...
    var path = new ROT.Path.AStar(player.getX(), player.getY(), function(x, y) {
      // If an entity is present at the tile, cant move there.
    ...
    // Once weve gotten the path, we want to move to the second cell that is
    // passed in the callback (the first is the entitys starting point)
    var count = 0;
  ...
  wander: function() {
    ...
    // Flip coin to determin if moving in x direction or y direction
...
Game.EntityMixins.GiantZombieActor = Game.extend(Game.EntityMixins.TaskActor, {
  ...
  canDoTask: function(task) {
    // If we havent already grown arm and HP <= 20, then we can grow.
    if (task === 'growArm') {
      return this.getHp() <= 20 && !this._hasGrownArm;
    // Spawn a slime only 10% of turns.
...
Game.EntityMixins.ExperienceGainer = {
  ...
  giveExperience: function(points) {
    ...
    var levelsGained = 0;
    // Loop until weve allocated all points.
    while (points > 0) {
  ...
  listeners: {
    onGainLevel: function() {
      ...
      while (this.getStatPoints() > 0) {
        ...
        var randStat = ROT.RNG.getItem(statOptions);
        return randStat;
